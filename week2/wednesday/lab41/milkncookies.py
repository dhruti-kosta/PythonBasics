...
def setcookie():
    ...
    if request.method == "POST":
        ...
        if request.form.get("favcolor"): # if nm was assigned via the POST
            favcolor = request.form.get("favcolor") # grab the value of nm from the POST
        else: # if a user sent a post without nm then assign value defaultuser
            favcolor = "black"

        if request.form.get("movie"): # if nm was assigned via the POST
            movie = request.form.get("movie") # grab the value of nm from the POST
        else: # if a user sent a post without nm then assign value defaultuser
        ...
        resp.set_cookie("favcolor", favcolor)
...
def getcookie():
    ...
    favcolor = request.cookies.get("favcolor") # preferred method
    movie = request.cookies.get("movie") # preferred method
    ...
    return f'<h1>Welcome {name}, Your favorite color is {favcolor} and favorite movie is {movie}</h1>'
